...
<script>
  ...
  export default {
    props: {
      ...
      // 使用裁切
      canCrop: {type: Boolean, default: false},
      // 裁切的最小宽度
    ...
    methods: {
      ...
      onFileChange(e) {
        ...
        xhr.response('post', this.uploadUrl);
        xhr.responseType = 'json';
        xhr.send(data);
        xhr.onload = function () {
          if (xhr.status === 200) {
            _this.editor.insertEmbed(_this.editor.getSelection().index, 'image', xhr.response.url);
          }
        }
      },
      // 裁切上传成功 res根据上传接口值获取
      ...
      imgClick() {
        if (this.canCrop) {
          ...
          input.accept = 'image/jpeg,image/png,image/jeg,image/gif';
    ...
    watch: {
      'value'(newVal, oldVal) {
        if (this.editor) {
          if (newVal !== this.content) this.content = newVal;
        }
      }
    }
  }
